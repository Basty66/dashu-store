      ...
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#fff8f5">
          ...
          <table style="width:100%;border-collapse:collapse;margin-bottom:24px">
            ...
            <tfoot>
              <tr><td colspan="2" style="padding:8px 12px;text-align:right;font-weight:600">Subtotal</td><td style="padding:8px 12px;text-align:right">$${((order.total + order.discount - (order.shippingCost || 0)) * 1000).toLocaleString('es-CL')}</td></tr>
              <tr><td colspan="2" style="padding:8px 12px;text-align:right">Envío</td><td style="padding:8px 12px;text-align:right">$${(order.shippingCost * 1000).toLocaleString('es-CL')}</td></tr>
              ${discountRow}
