...
export default function CartDrawer() {
  const { items, isOpen, setIsOpen, updateQuantity, removeItem, subtotal, freeShippingRemaining, freeShippingProgress, FREE_SHIPPING_THRESHOLD } = useCart()
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-navy/[0.04]">
              <span className="font-body font-semibold text-xs tracking-[0.15em] uppercase text-navy">Carrito</span>
            ...
            {/* Free shipping bar */}
            <div className="px-6 py-3 border-b border-navy/[0.03]">
              {freeShippingRemaining > 0 ? (
                <div className="flex items-center gap-3">
                  <div className="flex-1 shipping-bar">
                    <div className="shipping-bar-fill" style={{ width: `${freeShippingProgress}%` }} />
                  </div>
                  <p className="text-[11px] text-stone shrink-0">
                    ${freeShippingRemaining.toLocaleString('es-CL')} para envío gratis
                  </p>
                </div>
              ) : (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center">
                  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#8B8580" strokeWidth="1" className="mb-4">
                ...
                <div className="flex flex-col gap-3">
                  {items.map(item => (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: 20 }}
                      className="flex gap-4 p-3 bg-white/60 backdrop-blur-sm border border-white/50"
                    >
                      <div className="w-16 h-16 bg-ivory flex-shrink-0 overflow-hidden">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2.5 mt-2">
                          <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 flex items-center justify-center border border-navy/10 hover:bg-navy/5 transition-colors text-xs text-navy/60">−</button>
