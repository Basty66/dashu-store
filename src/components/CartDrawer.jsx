...
const spring = { type: 'spring', damping: 25, stiffness: 250, mass: 0.8 }
      ...
      {isOpen && (
        <>
          ...
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
            transition={spring}
            className="fixed top-0 right-0 h-full w-full max-w-md z-50 flex flex-col bg-cream/95 backdrop-blur-2xl shadow-2xl">
            ...
            <div className="px-6 py-3 border-b border-navy/[0.02]">
              {freeShippingRemaining > 0 ? (
                ...
                <motion.p initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="text-[11px] text-gold font-medium text-center">¡Envío gratis!</motion.p>
              )}
            </div>
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2.5 mt-2">
                            <motion.button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 flex items-center justify-center border border-navy/8 hover:bg-navy/5 transition-colors text-xs text-navy/50"
                        ...
                        <motion.button onClick={() => removeItem(item.id)} className="self-start p-1"
            ...
            {items.length > 0 && (
              <div className="px-6 py-5 border-t border-navy/[0.04]">
                ...
                <motion.button onClick={() => { setIsOpen(false); navigate('/checkout') }} className="btn-primary w-full justify-center text-xs"
